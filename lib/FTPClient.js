...
class FTPClient extends EventEmitter
{
  ...
  async uploadFile(localPath, remotePath, ensureDir = true)
  {
    if (!localPath)
    {
      throw new Error('Local path is required for uploadFile');
    }
    if (!remotePath)
    {
      throw new Error('Remote path is required for uploadFile');
    }
    return this._commands.uploadFile(localPath, remotePath, ensureDir);
  }
  ...
  async downloadFile(remotePath, localPath)
  {
    if (!remotePath)
    {
      throw new Error('Remote path is required for downloadFile');
    }
    if (!localPath)
    {
      throw new Error('Local path is required for downloadFile');
    }
    return this._commands.downloadFile(remotePath, localPath);
  }
  ...
  {
    return this._commands.ensureDir(path, recursive);
  }
